...
const PRODUCTOS_INICIALES: Producto[] = [
  { id: 1, nombre: 'Harina 0000',          unidad: 'kg', stock: 8,  stockMinimo: 15, leadTime: 3, ventaDiaria: 4,  proveedor: 'Molino San Martín'    },
  { id: 2, nombre: 'Queso Muzzarella',     unidad: 'kg', stock: 12, stockMinimo: 10, leadTime: 2, ventaDiaria: 6,  proveedor: 'Lácteos del Sur'       },
  { id: 3, nombre: 'Aceite de Girasol',    unidad: 'L',  stock: 5,  stockMinimo: 8,  leadTime: 4, ventaDiaria: 2,  proveedor: 'Aceitera Central'      },
  { id: 4, nombre: 'Carne Vacuna (Nalga)', unidad: 'kg', stock: 18, stockMinimo: 20, leadTime: 1, ventaDiaria: 10, proveedor: 'Frigorífico Oeste'     },
  { id: 5, nombre: 'Papa',                 unidad: 'kg', stock: 40, stockMinimo: 25, leadTime: 2, ventaDiaria: 8,  proveedor: 'Verdulería Mayorista'  },
]

const MOVIMIENTOS_INICIALES: Movimiento[] = [
  { id: 'm1', fecha: 'Hoy',         tipo: 'Salida',  productoId: 1, cantidad: 6,  motivo: 'Consumo de cocina'      },
  { id: 'm2', fecha: 'Ayer',        tipo: 'Salida',  productoId: 4, cantidad: 8,  motivo: 'Consumo de cocina'      },
  { id: 'm3', fecha: 'Ayer',        tipo: 'Entrada', productoId: 2, cantidad: 15, motivo: 'Reposición proveedor'   },
  { id: 'm4', fecha: 'Hace 2 días', tipo: 'Salida',  productoId: 3, cantidad: 3,  motivo: 'Consumo de cocina'      },
  { id: 'm5', fecha: 'Hace 2 días', tipo: 'Salida',  productoId: 5, cantidad: 6,  motivo: 'Consumo de cocina'      },
  { id: 'm6', fecha: 'Hace 3 días', tipo: 'Entrada', productoId: 1, cantidad: 20, motivo: 'Reposición proveedor'   },
  { id: 'm7', fecha: 'Hace 4 días', tipo: 'Salida',  productoId: 4, cantidad: 12, motivo: 'Consumo de cocina'      },
]
...
function calcEstado(p: Producto): Estado {
  ...
  if (dias <= p.leadTime + 2) return 'Pedir pronto'
  return 'OK'
}
function sugerencia(p: Producto): string {
  ...
  if (estado === 'Quiebre' || estado === 'Pedido urgente') {
    return `Pedí hoy — el proveedor tarda ${p.leadTime} día${p.leadTime === 1 ? '' : 's'} y el insumo se agota en ${Math.round(dias)} día${Math.round(dias) === 1 ? '' : 's'}`
  }
  ...
  return `Tenés para ${Math.round(dias)} días · Podés esperar hasta ${fechaStr} para pedir`
}
...
function mailtoPedido(p: Producto) {
  const subject = encodeURIComponent('Solicitud de reposición de insumo')
  const body = encodeURIComponent(
    `Insumo: ${p.nombre} | Cantidad sugerida: ${cantidadSugerida(p)}${p.unidad} | Proveedor: ${p.proveedor}`
  )
  ...
  return (
    <div style={{ backgroundColor: '#1B5BC1' }} className="flex items-center justify-between px-5 py-3 shrink-0">
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-white/75 text-sm">Gastronomía · Stock de insumos</span>
      </div>
      <button onClick={onBack}
  ...
  return (
    <Modal open={!!producto} onClose={onClose} title={`Registrar entrada — ${producto.nombre}`}>
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-[#5A6871] mb-1.5">Cantidad recibida ({producto.unidad})</label>
          <input
  ...
  return (
    <div className="rounded-xl p-4" style={{ backgroundColor: '#FDECEC', border: '1px solid #f5b5b5' }}>
      <p className="text-sm font-semibold" style={{ color: '#C0392B' }}>
        ⚠️ Tenés {count} insumo{count === 1 ? '' : 's'} que necesita{count === 1 ? '' : 'n'} atención inmediata antes de que te falte en pleno servicio. Revisá las sugerencias de reposición.
      </p>
    </div>
  )
}
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-[#DCE5E9] p-4 shadow-sm">
          <p className="text-xs text-[#5A6871] mb-1.5">Insumos en quiebre</p>
        ...
        <div className="bg-white rounded-xl border border-[#DCE5E9] p-4 shadow-sm">
          <p className="text-xs text-[#5A6871] mb-1.5">Insumos OK</p>
            ...
            {estados.map(({ p, estado }) => (
              <tr key={p.id} className="border-t border-[#DCE5E9] align-top">
                ...
                <td className="py-3 px-3 text-right">
                  <p className="tabular-nums font-bold text-[#353C42]">{p.stock}{p.unidad}</p>
                  <p className="text-xs text-[#5A6871]">mín. {p.stockMinimo}{p.unidad}</p>
                </td>
...
function RegistrarMovimientoModal({
  ...
  return (
    <Modal open={open} onClose={onClose} title="Registrar movimiento">
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-[#5A6871] mb-1.5">Insumo</label>
          <select
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ej: Consumo de cocina"
            className="w-full border border-[#DCE5E9] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#306ECF]"
          />
        </div>
        <div className="flex justify-end gap-2 pt-2">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-wide text-[#5A6871] bg-[#F3F6F5]">
              ...
              <th className="py-2.5 px-4 font-semibold">Insumo</th>
